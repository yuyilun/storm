...
import com.hadoop.storm.trident.functions.MySplit2;
...
public class TridentPVTopo {
	...
	public static StormTopology buildTopology(LocalDRPC drpc) {
		...
		StateFactory state = HBaseAggregateState.transational(config);

		TridentTopology topology = new TridentTopology();
		topology.newStream("spout1", spout)
				.each(new Fields("eachLog"), new MySplit2("\t"), new Fields("date","cf","pv_count", "session_id"))
				.project(new Fields("date","cf","pv_count"))//投影操作是对数据上进行列裁剪。
				.groupBy(new Fields("date","cf","pv_count"))//输出将会是被group的字段以及聚合器输出的字段
				/**
